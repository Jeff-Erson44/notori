<div id="bar"></div>
<div class="pointer"></div>
<div class="pointer2"></div>
<a data-link="hero" class="scroll_to scrollToTop"><i class="fas fa-arrow-up"></i></a>
<header>
    <nav id="navbar">
        <div>
            <figure>
                <a href="index.php"><img src="assets/img/logo_default.svg" alt="logo du site imminence" class="logo"></a>
            </figure>
        </div>

        <button class="mobile__nav--toggle" aria-controls="primary-navigation" aria-expanded="false">
                <span class="sr-only"></span>
        </button>

        <ul id="primary-navigation" data-visible="false" class="primary-navigation">
            <li class="active">
                <div>
                    <h4 class="title">
                        <a data-link="apropos" class="scroll_to">
                            <div class="bg__links--text">
                                <span class="nav-links">À &nbsp;propos</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
            <li class="active">
                <div>
                    <h4 class="title">
                        <a data-link="pourvous" class="scroll_to" >
                            <div class="bg__links--text">
                                <span class="nav-links">Pour Vous</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
            <li class="active">
                <div>
                    <h4 class="title">
                        <a data-link="faq" class="scroll_to">
                            <div class="bg__links--text">
                                <span class="nav-links">FAQ</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
            <li class="active navMobile">
                <div>
                    <h4 class="title">
                        <a target="_blank" href="https://www.facebook.com/imminenceentrepreneur">
                            <div class="bg__links--text">
                                <span class="nav-links">Facebook</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
            <li class="active navMobile">
                <div>
                    <h4 class="title">
                        <a target="_blank" href="https://www.linkedin.com/company/declic1">
                            <div class="bg__links--text">
                                <span class="nav-links">LinkedIn</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
            <li class="active navMobile">
                <div>
                    <h4 class="title">
                        <a target="_blank" href=" https://www.instagram.com/imminence.entrepreneur/">
                            <div class="bg__links--text">
                                <span class="nav-links">Instagram</span>
                            </div>
                        </a>
                    </h4>
                </div>
            </li>
        </ul>
    </nav>
</header>